...
type SSOProvider struct {
	...
	EnableQRLogin bool   `json:"enableQrLogin"`
}

// GetSSOProviders 获取可用的 SSO 免登列表（公开接口）
func GetSSOProviders(c *gin.Context) {
	var connectors []models.Connector
	storage.DB.Where("(im_enable_sso = ? OR im_enable_qr_login = ?) AND status = 1", true, true).
	...
	for _, conn := range connectors {
		...
		p := SSOProvider{
			...
			EnableQRLogin: conn.IMEnableQRLogin,
		}
...
// completeIMLogin 完成 IM 平台登录的通用逻辑
func completeIMLogin(c *gin.Context, conn models.Connector, imUser *imclient.IMUserInfo, loginMethod string) {
	// 匹配本地用户
	var localUser models.User
	found := false
	...
	if !found && conn.IMAutoRegister {
		...
		found = true
		log.Printf("[%s] 自动创建用户: %s", loginMethod, username)
	}
	...
	log.Printf("[%s] 登录成功: %s (%s)", loginMethod, localUser.Username, imUser.Name)
...
// DingTalkQRLogin 钉钉扫码登录
func DingTalkQRLogin(c *gin.Context) {
	var req struct {
		ConnectorID uint   `json:"connectorId" binding:"required"`
	...
	if conn.Type != "im_dingtalk" {
		respondError(c, http.StatusBadRequest, "该连接器不支持钉钉扫码登录")
		return
	}

	if !conn.IMEnableSSO {
		respondError(c, http.StatusForbidden, "该连接器未启用SSO登录")
		return
	}

	// 创建钉钉客户端
	dtClient := imclient.NewDingTalkClient(conn)

	// 通过 OAuth2 授权码获取用户信息（DTFrameLogin 新版流程）
	imUser, err := dtClient.GetUserByOAuth2Code(req.Code)
	if err != nil {
		log.Printf("[钉钉扫码] OAuth2 授权码验证失败: %v, 尝试旧版 SNS 流程", err)
		// 降级到旧版 SNS 流程
		imUser, err = dtClient.GetUserBySNSAuthCode(req.Code)
		if err != nil {
			log.Printf("[钉钉扫码] SNS 授权码验证也失败: %v", err)
			respondError(c, http.StatusUnauthorized, "扫码验证失败: "+err.Error())
			return
		}
	}

	completeIMLogin(c, conn, imUser, "钉钉扫码登录")
}

// FeishuQRLogin 飞书扫码登录
func FeishuQRLogin(c *gin.Context) {
	var req struct {
		ConnectorID uint   `json:"connectorId" binding:"required"`
		Code        string `json:"code" binding:"required"`
	}
	...
	if conn.Type != "im_feishu" {
		respondError(c, http.StatusBadRequest, "该连接器不支持飞书扫码登录")
		return
	}
	...
	// 创建飞书客户端
	fsClient := imclient.NewFeishuClient(conn)

	// 通过 OAuth 授权码获取用户信息
	imUser, err := fsClient.GetUserByAuthCode(req.Code)
	if err != nil {
		log.Printf("[飞书扫码] 授权码验证失败: %v", err)
		respondError(c, http.StatusUnauthorized, "扫码验证失败: "+err.Error())
		return
	}

	completeIMLogin(c, conn, imUser, "飞书扫码登录")
}

// WeChatWorkQRLogin 企业微信扫码登录
func WeChatWorkQRLogin(c *gin.Context) {
	var req struct {
		ConnectorID uint   `json:"connectorId" binding:"required"`
		Code        string `json:"code" binding:"required"`
	}
	...
	if conn.Type != "im_wechatwork" {
		respondError(c, http.StatusBadRequest, "该连接器不支持企业微信扫码登录")
		return
	}
	...
	// 创建企微客户端
	wxClient := imclient.NewWeChatWorkClient(conn)

	// 通过 OAuth 授权码获取用户信息
	imUser, err := wxClient.GetUserByAuthCode(req.Code)
	if err != nil {
		log.Printf("[企微扫码] 授权码验证失败: %v", err)
		respondError(c, http.StatusUnauthorized, "扫码验证失败: "+err.Error())
		return
	}

	completeIMLogin(c, conn, imUser, "企微扫码登录")
}
