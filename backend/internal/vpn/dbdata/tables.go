...
type AddressGroup struct {
	...
	Addresses []ValData `json:"addresses" xorm:"Text"`
...
type Group struct {
	...
	RouteIncludeGroupIDs []int              `json:"route_include_group_ids" xorm:"Text"`
	RouteExcludeGroupIDs []int              `json:"route_exclude_group_ids" xorm:"Text"`
	...
	// 使用范围控制
	AllowedUserIDs  []uint `json:"allowed_user_ids" xorm:"Text"`  // 允许的用户 ID 列表
	AllowedGroupIDs []uint `json:"allowed_group_ids" xorm:"Text"` // 允许的部门/用户组 ID 列表
	CreatedAt       time.Time              `json:"created_at" xorm:"DateTime created"`
	UpdatedAt       time.Time              `json:"updated_at" xorm:"DateTime updated"`
}
...
type User struct {
	...
	Phone    string `json:"phone" xorm:"varchar(20)"` // 手机号，用于短信验证
	...
	DisableSms bool       `json:"disable_sms" xorm:"Bool"` // 禁用短信验证
...
type UserActLog struct {
	...
	AuthType        string    `json:"auth_type" xorm:"varchar(32) not null default ''"`
...
type AccessAudit struct {
	...
	DstHost     string    `json:"dst_host" xorm:"varchar(255) not null default ''"` // 目的域名
