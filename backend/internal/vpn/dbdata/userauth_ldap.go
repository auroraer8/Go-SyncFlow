...
type AuthLdap struct {
	...
	PhoneAttr   string `json:"phone_attr"` // LDAP中存储手机号的属性名
}
...
func (auth AuthLdap) checkUser(name, pwd string, g *Group, ext map[string]interface{}) error {
	...
	defer con.Close()
	// 连接LDAP
	ldapURL := "ldap://" + auth.Addr
	if auth.Tls {
		ldapURL = "ldaps://" + auth.Addr
	}
...
// GetLdapUserPhone 从LDAP获取用户的手机号
func GetLdapUserPhone(name, group string) (string, error) {
	g := &Group{}
	...
	if !ok || authType != "ldap" {
		return "", errors.New("group is not using ldap auth")
	}
	ldapData, ok := g.Auth["ldap"]
	if !ok {
		return "", errors.New("ldap config not found")
	}
	bodyBytes, err := json.Marshal(ldapData)
	if err != nil {
		return "", err
	}
	var auth AuthLdap
	...
	if auth.PhoneAttr == "" {
		return "", errors.New("phone attribute not configured")
	}
	ldapURL := "ldap://" + auth.Addr
	if auth.Tls {
		ldapURL = "ldaps://" + auth.Addr
	...
	if len(sr.Entries) != 1 {
		return "", errors.New("user not found or multiple users found")
	}
	phone := sr.Entries[0].GetAttributeValue(auth.PhoneAttr)
	return phone, nil
}
