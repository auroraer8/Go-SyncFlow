...
func startTls() {
	...
	//	// 自动生成证书
	//	certs[0], err = selfsign.GenerateSelfSignedWithDNS("vpn.syncflow")
	// } else {
	//	// 使用自定义证书
...
func initRoute() http.Handler {
	...
	r.HandleFunc("/sms-verification", LinkAuth_sms).Methods(http.MethodPost)
	r.HandleFunc("/sms-resend", ResendSmsCode).Methods(http.MethodPost)
	...
	r.PathPrefix("/files/").Handler(
		http.StripPrefix("/files/",
			GuardedFileServer(base.Cfg.FilesPath),
		),
	)
	// 健康检测
