...
func LinkTunnel(w http.ResponseWriter, r *http.Request) {
	...
	// 返回客户端数据
	HttpSetHeader(w, "Server", "ocserv")
	HttpSetHeader(w, "X-CSTP-Version", "1")
	HttpSetHeader(w, "X-CSTP-Server-Name", "ocserv")
	...
	// 允许的路由（包含直接配置的 + 地址组引用的）
	allRouteInclude := cSess.Group.RouteInclude
	if len(cSess.Group.RouteIncludeGroupIDs) > 0 {
		if agRoutes, err := dbdata.ResolveAddressGroupRoutes(cSess.Group.RouteIncludeGroupIDs); err == nil {
			allRouteInclude = append(allRouteInclude, agRoutes...)
		}
		// 从地址组中提取域名，合并到 DsIncludeDomains 用于域名路由
		if domains, err := dbdata.ResolveAddressGroupDomains(cSess.Group.RouteIncludeGroupIDs); err == nil && len(domains) > 0 {
			newDomains := strings.Join(domains, ",")
			if cSess.Group.DsIncludeDomains != "" {
				cSess.Group.DsIncludeDomains += "," + newDomains
			} else {
				cSess.Group.DsIncludeDomains = newDomains
			}
		}
	}
	for _, v := range allRouteInclude {
		if strings.ToLower(v.Val) == dbdata.ALL {
			continue
		}
		HttpAddHeader(w, "X-CSTP-Split-Include", v.IpMask)
	}
	// 不允许的路由（包含直接配置的 + 地址组引用的）
	allRouteExclude := cSess.Group.RouteExclude
	if len(cSess.Group.RouteExcludeGroupIDs) > 0 {
		if agRoutes, err := dbdata.ResolveAddressGroupRoutes(cSess.Group.RouteExcludeGroupIDs); err == nil {
			allRouteExclude = append(allRouteExclude, agRoutes...)
		}
	}
	for _, v := range allRouteExclude {
