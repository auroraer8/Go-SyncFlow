...
func initCmd() {
	...
	rootCmd = &cobra.Command{
		Use:   "syncflow-vpn",
		Short: "Go-SyncFlow VPN Server",
		Long:  `Go-SyncFlow VPN is a enterprise-grade VPN Server`,
...
func initToolCmd() *cobra.Command {
	toolCmd := &cobra.Command{
		Use:   "tool",
		Short: "Go-SyncFlow VPN tool",
		Long:  `Go-SyncFlow VPN tool application`,
	}
	...
	toolCmd.Run = func(cmd *cobra.Command, args []string) {
		...
		case otp:
			...
			fmt.Printf("Otp:%s\n\n", s)
			qrstr := fmt.Sprintf("otpauth://totp/%s:%s?issuer=%s&secret=%s", "syncflow_vpn_admin", "admin@syncflow-vpn", "syncflow_vpn_admin", s)
		...
		case debug:
			// linkViper.Debug()
		default:
			fmt.Println("Using [syncflow-vpn tool -h] for help")
		}
	}
